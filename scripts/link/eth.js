...
async function changeLINKEthManagerThreshold(managerAddr, threshold) {
  const web3 = new Web3(process.env.ETH_NODE_URL);
  let ethUserAccount = web3.eth.accounts.privateKeyToAccount(
    process.env.ETH_MASTER_PRIVATE_KEY
  );
  ...
  const contractJson = require("../../build/contracts/LINKEthManager.json");
...
async function authorizeLINKEth(managerAddr, userAddr) {
  const web3 = new Web3(process.env.ETH_NODE_URL);
  let ethUserAccount = web3.eth.accounts.privateKeyToAccount(
    process.env.ETH_MASTER_PRIVATE_KEY
  );
