...
async function deployLockableHmyNFTManager(wallet) {
...
async function deployMintableNFTManager(wallet) {
    const hmyNFTManagerJson = require("../../build/contracts/NFTEthManager.json");
    let hmyNFTManagerContract = hmy.contracts.createContract(hmyNFTManagerJson.abi);
    hmyNFTManagerContract.wallet.setSigner(process.env.HMY_ADMIN);
    ...
    let response = await hmyNFTManagerContract.methods
        ...
        .send(options);
    const managerAddr = response.transaction.receipt.contractAddress;
    console.log("MintableHmyManager contract deployed at " + managerAddr);
    return managerAddr;
}
...
async function deployBridgeAirdrop() {
    const hmyBridgeAirdropJson = require("../../build/contracts/BridgeAirdrop.json");
    let hmyBridgeAirdropContract = hmy.contracts.createContract(hmyBridgeAirdropJson.abi);
    hmyBridgeAirdropContract.wallet.setSigner(process.env.HMY_ADMIN);

    let deployOptions = {data: hmyBridgeAirdropJson.bytecode, arguments: []};
    ...
    let response = await hmyBridgeAirdropContract.methods
        .contractConstructor(deployOptions)
        .send(options);
    const bridgeAirdropAddr = response.transaction.receipt.contractAddress;
    console.log("BridgeAirdrop contract deployed at " + bridgeAirdropAddr);
    return bridgeAirdropAddr;
}
